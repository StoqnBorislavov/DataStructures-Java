...
public class BinaryTree {
    ...
    public Integer findLowestCommonAncestor(int first, int second) {
        ...
        int smallestAncestor = firstPath.size() < secondPath.size() ? firstPath.size() : secondPath.size();
        int i = 0;
        for (; i < smallestAncestor; i++) {
    ...
    public List<Integer> topView() {
        ...
        return offsetToValueLevel.values().stream().map(Pair::getKey).collect(Collectors.toList());
    }
    ...
    private void traverseTree(BinaryTree binaryTree, int offset, int level,
        ...
        if(currentValueLevel == null){
            offsetToValueLevel.put(offset, new Pair<>(binaryTree.value, level));
        } else if (level < currentValueLevel.getValue()){
            offsetToValueLevel.put(offset, new Pair<>(binaryTree.value, level));
        }
